...
export default function ItemSelection({
  ...
  const handleOtherKeyDown = (event: KeyboardEvent<HTMLInputElement>) => {
    if (event.key === 'Enter' && otherValue.trim()) {
        ...
        const newSelectedItems = [...selectedItems, otherValue.trim()];
        onSelectedItemsChange(newSelectedItems);
        setOtherValue('');
      ...
      <div className="flex flex-wrap gap-3">
        {selectedItems.map((item) => (
             <Badge
                ...
                className={cn(
                  "cursor-pointer px-5 py-2.5 text-lg rounded-full transition-all duration-200 border-2",
        ...
        {showOtherInput ? (
            <Input 
                ...
                className="h-auto px-5 py-2.5 text-lg rounded-full w-48 border-2 border-primary"
                autoFocus
            />
        ) : (
            <Badge
                variant="default"
                className="cursor-pointer px-5 py-2.5 text-lg rounded-full border-dashed border-2 border-primary bg-primary text-primary-foreground"
