...
const qualificationPlaceholders: Record<string, string[]> = {
    ...
    'Professional Certificate (e.g., NPTEL, Coursera)': ['e.g., AI/ML Certification', 'e.g., Full-Stack Development Bootcamp'],
...
export default function InterestsClientPage() {
  ...
  const handleGenerateSkills = useCallback(async () => {
    ...
    setTimeout(() => {
        setSuggestedSkills(["Project Management", "Agile Methodologies", "Client Communication", "Budgeting", "Team Leadership"]);
  ...
  const handleGenerateSubjects = useCallback(async () => {
    ...
    // Use hardcoded example data instead of AI generation
    setTimeout(() => {
        setSuggestedSubjects(["Physics", "Chemistry", "Mathematics", "Computer Science", "Engineering Basics"]);
        if (formStep !== 'subjects') {
            setFormStep('subjects');
        }
        setIsGenerating(false);
    }, 500);
  }, [interestsInput, formStep]);
  ...
  const getActionButtons = () => {
    const isSubmitAction = isStudent ? (formStep === 'skills' || formStep === 'subjects') : true;
    let clickHandler = isSubmitAction ? handleSubmit : handleNextStep;
    
    let buttonText = 'Next';
    if(isStudent) {
        if(isClass12OrBelow) {
            if(formStep === 'subjects') buttonText = 'Submit';
            if (formStep === 'interests') {
        ...
        {formStep === 'qualification' && !isClass12OrBelow && (
          <div className='w-full'>
            <Input
              ...
              maxLength={60}
              className={cn(
                'text-left text-2xl md:text-3xl h-auto p-0 bg-transparent border-none focus-visible:ring-0 focus-visible:ring-offset-0 transition-all placeholder-fade',
                'placeholder:text-muted-foreground/30',
                isPlaceholderFading && 'placeholder-fade-out'
              )}
              autoFocus
            />
  ...
  const renderProfessionalForm = () => (
    <div className="w-full animate-in fade-in slide-in-from-bottom-5 duration-700">
        <div>
            ...
            {organization && (
                <h2 className="text-xl md:text-2xl text-muted-foreground capitalize mt-1">
                    at {organization}
                </h2>
            )}
        </div>
    ...
    <div className="flex flex-col min-h-dvh bg-transparent">
      ...
      <main className="flex-1 flex items-center justify-center p-4">
        <div className="w-full max-w-4xl p-8 md:p-12">
            {isStudent ? renderStudentForm() : renderProfessionalForm()}
        </div>
      </main>
      <Footer />
    </div>
